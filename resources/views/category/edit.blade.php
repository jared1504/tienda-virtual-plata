...
<x-dashboard :items="$items">
    <h1 class='font-black text-4xl text-blue-900'>Actualizar Categoría</h1>
    <p class="mt-3 text-lg">Llena los siguientes campos para actualizar una Categoría</p>

    <form class=' my-5 p-5 w-3/4 mx-auto  rounded-lg bg-white drop-shadow-2xl' action="{{route('category.update',$category)}}"
        method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <p class="text-2xl text-center font-bold mb-5">Llena los campos para Actualizar una Categoría</p>
        <div class="my-4">
            <label class='text-gray-800 text-xl mt-5' for="name">Nombre</label>
            <input class="mt-2 block w-full p-3 bg-gray-100 text-lg focus:outline-none" value="{{$category->name}}"
                type="text" name="name" id="name" placeholder="Nombre de la Categoría">
            @error('name') <div
